...
from horde.jobs import InMemoryJobQueue, SupabaseJobQueue
...
class Worker:
    def __init__(
        self,
        agent: Agent,
        queue: InMemoryJobQueue | SupabaseJobQueue,
        scopes,
    ...
    async def run_once(self) -> bool:
        await self._resolve(self.queue.recover_expired())
        job = await self._resolve(
            self.queue.claim(str(self.agent.id), lease_seconds=max(30, int(self.settings.tool_timeout_seconds * 2)))
        )
        if job is None:
        ...
        if not decision.allowed:
            await self._resolve(self.queue.fail(job.id, decision.reason, retry=False))
            ...
            return True
        try:
            await self._resolve(self.queue.mark_running(job.id))
            ...
            if result.ok:
                await self._resolve(self.queue.complete(job.id))
                self.lifecycle.cycle_complete(self.agent, True)
                event = "job_succeeded"
            else:
                await self._resolve(self.queue.fail(job.id, result.error or "tool failed"))
                self.lifecycle.cycle_complete(self.agent, False)
                event = "job_failed"
            ...
            await self._resolve(self.queue.fail(job.id, str(exc)))
            self.lifecycle.cycle_complete(self.agent, False)
